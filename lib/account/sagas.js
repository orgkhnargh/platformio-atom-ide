...
import { ACCOUNT_INFORMATION_KEY, IS_ACCOUNT_INFORMATION_UPDATE_IN_PROGRESS_KEY, IS_AUTH_REQUEST_IN_PROGRESS_KEY, IS_LOGGED_IN_KEY, LAST_USED_USERNAME_KEY } from './selectors';
import { FORGOT_FORM, LOGIN_FORM, REGISTER_FORM } from './containers/auth-container';
import { call, fork, put, take, takeEvery } from 'redux-saga/effects';
import { deleteError, updateEntity, updateError, updateFlagValue, updateInputValue } from '../core/actions';
...
function* watchLoginRequest() {
  yield takeEvery(actions.LOGIN_REQUEST, function*({ username, password }) {
    try {
      ...
      yield put(updateFlagValue(IS_AUTH_REQUEST_IN_PROGRESS_KEY, true));
      ...
      yield put(updateFlagValue(IS_AUTH_REQUEST_IN_PROGRESS_KEY, false));
  ...
  while (true) {
    try {
      ...
      if (data && data.username) {
        yield put(updateFlagValue(IS_LOGGED_IN_KEY, true));
        yield put(updateInputValue(LAST_USED_USERNAME_KEY, data.username));
      }
    } catch (error) {
      console.error(error);
    } finally {
